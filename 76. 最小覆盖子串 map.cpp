...
{
public:
    unordered_map<char, int> hashmap, hashmapTemp;
    bool check()
    {
        /*for (auto Apair : hashmapTemp)
        {
            if (hashmap[Apair.first] > Apair.second)
                return 0;是錯誤的
        }*/
        for (auto Apair : hashmap)
        {
            if (Apair.second > hashmapTemp[Apair.first])
                return 0;
        }
        return 1;
    }
